...
return view.extend({
	render: function () {
		...
		function extractDDNSGoMessage(line) {
			if (!line || !line.includes('ddns-rs')) return null;
		...
		function formatLogContent(logContent) {
			if (!logContent || logContent.trim() === '') {
				return E('div', { 'class': 'log-line' }, _('No ddns-rs logs found.'));
			}
			...
			for (var i = 0; i < lines.length; i++) {
				...
				if (line === '' || line.includes('No ddns-rs logs found')) continue;
		...
		function clearLogs(button) {
			...
			return fs.exec('/usr/libexec/ddns-rs-call', ['clear_logs'])
				.then(function(res) {
		...
		function fetchLogs() {
			...
			return fs.exec('/usr/libexec/ddns-rs-call', ['get_logs'])
				.then(function(res) {
					var logContent = '';
					...
					var lineCount = logContent.split('\n').filter(l => 
						l.trim() !== '' && !l.includes('No ddns-rs logs found')
					).length;
