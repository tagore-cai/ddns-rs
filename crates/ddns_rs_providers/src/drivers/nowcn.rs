...
const BASE_URL: &str = "https://api.now.cn";
...
struct NowcnRecordListResp {
    ...
    data: Vec<NowcnRecord>,
}

#[derive(serde::Deserialize, Debug)]
struct NowcnBaseResult {
    #[serde(rename = "RequestId", default)]
    request_id: String,
    #[serde(rename = "Id", default)]
    id: i32,
...
impl Nowcn {
    ...
    async fn create(&self, domain: &mut Domain, record_type: &str, ip_addr: &str) {
        ...
        let result: NowcnBaseResult = match res {
            Err(e) => {
    ...
    async fn modify(&self, record: NowcnRecord, domain: &mut Domain, record_type: &str, ip_addr: &str) {
        // 相同不修改
    ...
    async fn get_record_list(&self, domain: &Domain, typ: &str) -> Result<NowcnRecordListResp, String> {
        let mut param = BTreeMap::new();
        param.insert("Domain".to_string(), domain.domain_name.clone());
...
#[async_trait]
impl crate::engine::DnsProvider for Nowcn {
